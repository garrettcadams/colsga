;( function( $ ){
	...
	var lava_directory_form_script = function() { if( ! this.loaded ) this.init(); }

	lava_directory_form_script.prototype.constructor = lava_directory_form_script;
	lava_directory_form_script.prototype.init = function(){
		var obj = this;

		obj.loaded	 = 1;
		obj.attr = lava_directory_manager_admin_meta_args;
		obj.geocoder = new google.maps.Geocoder();

		obj.setSortableDetailImages();
	...
	lava_directory_form_script.prototype.item_meta = function(){
		var obj			= this;
				...
				, marker:{
					...
					, events:{
						position_changed: function( m )
						{
							...
							obj.update_address(m.getPosition());
	...
	lava_directory_form_script.prototype.update_address = function( latlng, _address ) {
		var
			obj = this,
		...
		obj.geocoder.geocode(query, function( results, state ) {
			...
			if( state == google.maps.GeocoderStatus.OK ) {
				...
				$("input[name='lava_pt[map][address]']").val(JSON.stringify(results[0].address_components));

				$.each( results[0].address_components, function( iResult, arrAddress ) {

					if( arrAddress.types[0] === 'country' ) {
						$("input[name='lava_pt[map][country]']").val( arrAddress.long_name );
					}

					if( arrAddress.types[0] === 'locality' ) {
						$("input[name='lava_pt[map][locality]']").val( arrAddress.long_name );
					}

					if( arrAddress.types[0] === 'postal_code' ) {
						$("input[name='lava_pt[map][zipcode]']").val( arrAddress.long_name );
					}

					if( arrAddress.types[1] === 'sublocality' ){
						if( arrAddress.types[2] === 'sublocality_level_1' ){
							$("input[name='lava_pt[map][political]']").val( arrAddress.long_name );
						}else if( arrAddress.types[2] === 'sublocality_level_2' ) {
							$("input[name='lava_pt[map][political2]']").val( arrAddress.long_name );
						}
					}
				} );
			}
	...
	lava_directory_form_script.prototype.toggle_streetview = function(){
		var obj		= this;
	...
	lava_directory_form_script.prototype.type_latLng = function(){
		var obj = this;
		return function( e ) {
			e.preventDefault();
	...
	lava_directory_form_script.prototype.setSortableDetailImages = function() {
		if(typeof jQuery.fn.sortable == 'undefined') {
			return;
		}
		var $detailWrap = $('.lava_pt_images');
		$detailWrap.sortable({
			placeholder: 'lava-pt-images-holder',
		}).disableSelection();
	}

	new lava_directory_form_script;
} )( jQuery );