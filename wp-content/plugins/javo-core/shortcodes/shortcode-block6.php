<?php
class jvbpd_block6 extends Jvbpd_Shortcode_Parse
{
	...
	public function loop( $queried_posts )
	{
		...
		if( ! empty( $queried_posts ) ) : foreach( $queried_posts as $index => $post ) {
			...
			$objModuleSmallGrid	= new moduleSmallGrid( $post );
			switch( $columns ) :
				case 2: echo "<div class='col-md-6 col-sm-6 col-xs-6'>"; break;
				case 3: echo "<div class='col-md-4 col-sm-6 col-xs-6'>"; break;
				case 1:
				default: echo "<div class='col-md-12'>";
			endswitch;
				echo $objModuleSmallGrid->output();
			echo "</div>";
		} endif;
		$this->sParams();
		$this->pagination();
