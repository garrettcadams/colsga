<?php
class jvbpd_login2 extends Jvbpd_Shortcode_Parse
{
	...
	public function initialize()
	{
		...
		if( function_exists( 'jvbpd_getCurrentUserPage' ) ) {
			$this->mypage		= jvbpd_getCurrentUserPage();
			$this->editProfile = jvbpd_getCurrentUserPage( 'edit-my-profile' );
			$this->submit_item	= jvbpd_getCurrentUserPage( 'add-lv_listing');
		}

		$this->user_avatar	= str_replace(
			'>', 'data-no-lazy>',
			get_avatar( $this->user_id, 96 )
		);
	}
	...
	public function panel_body() {
		...
		<div class="meta-container">
			<div class="col-md-3 meta-avatar">
			...
			<div class="col-md-2 meta-actions">
				<a href="<?php echo $this->logout_page; ?>" title="<?php _e( "Logout", 'jvfrmtd' ); ?>" class="btn btn-default btn-xs btn-block btn-logout" target="_self">
					<?php _e( "Logout", 'jvfrmtd' ); ?>
				</a>
				<a href="<?php echo $this->submit_item; ?>" title="<?php _e( "Submit", 'jvfrmtd' ); ?>" class="btn btn-default btn-block btn-submit-item" target="_self">
					<?php _e( "Submit", 'jvfrmtd' ); ?>
				</a>
			</div>
		</div><!-- /.row -->
		<?php
