...
class Jvbpd_Absolute_Images extends Widget_Base {

	public function get_name() { return 'jvbpd-absolute-images'; }
	public function get_title() { return 'Absolute images'; }
	...
	protected function _register_controls() {
		...
		$repeater = new \Elementor\Repeater();
			...
			$repeater->start_controls_tab( 'content', [ 'label' => __( 'Content', 'jvfrmtd' ) ] );
				...
				$repeater->add_control(
					...
					[
						...
						'default' => 'dot',
						'options' => [
							'dot'  => __( 'Dot & hide content', 'jvfrmtd' ),
							'show-content' => __( 'Show Content', 'jvfrmtd' ),
						],
					]
				);
				...
				$repeater->add_group_control(
					Group_Control_Image_Size::get_type(),
					[
						'name' => 'image', // Usage: `{name}_size` and `{name}_custom_dimension`, in repeater case `image_size` and `image_custom_dimension`.
						'default' => 'large',
						'separator' => 'none',
					]
				);
			...
			$repeater->start_controls_tab( 'position', [ 'label' => __( 'Position', 'jvfrmtd' ) ] );
				...
				$repeater->add_control(
					'hotspot_right',
					[
						'label' => __( 'Right position', 'jvfrmtd' ),
						'type' => Controls_Manager::SLIDER,
						'size_units' => [ '%', 'px' ],
						'range' => [
							'px' => [
						...
						'selectors' => [
							'{{WRAPPER}} {{CURRENT_ITEM}}.hotspot_item' => 'right:{{SIZE}}{{UNIT}}',
						]
					]
				);
				...
				$repeater->add_control(
					'hotspot_bottom',
					[
						'label' => __( 'Bottom position', 'jvfrmtd' ),
						'type' => Controls_Manager::SLIDER,
						'size_units' => [ '%', 'px' ],
						'range' => [
							'px' => [
						...
						'selectors' => [
							'{{WRAPPER}} {{CURRENT_ITEM}}.hotspot_item' => 'bottom:{{SIZE}}{{UNIT}}',
						]
					]
				);
				...
				$repeater->add_control(
					...
					[
						...
						'default' => 0,
						'selectors' => [
							'{{WRAPPER}} .heartbeat' => 'z-index: {{VALUE}}',
							'{{WRAPPER}} .detail-tooltip' => 'z-index: {{VALUE}}',
						],
					]
				);
			$repeater->end_controls_tab();
		...
		$this->add_control(
			'des_init_border_color',
			[
				'label' => __( 'Tooltip Border color', 'jvfrmtd' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'default' => '#eee',
				'selectors' => [
					'{{WRAPPER}} .detail-tooltip' => 'border: 1px solid {{VALUE}}',
				],
			]
		);
	...
	protected function render() {
		...
		if ( $settings['list'] ) {
			...
			foreach (  $settings['list'] as $item ) {
				echo '<div class="hotspot_item elementor-repeater-item-' . $item['_id'] . ' '. $item['hotspot_type'] .'">';
					echo '<div class="heartbeat"></div>';
						...
						if ( $item['list_image']){
							echo Group_Control_Image_Size::get_attachment_image_html( $item, 'list_image' );
						}
						echo '</div>';
				echo '</div>';
			}
	...
	protected function _content_template() {
		...
		<# if ( settings.list.length ) { #>
			...
			<# _.each( settings.list, function( item ) { #>
				...
				var item_image_url = elementor.imagesManager.getImageUrl( item_image );
				#>
				<div class="hotspot_item elementor-repeater-item-{{ item._id }} {{ item.hotspot_type }}">
