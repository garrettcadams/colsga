 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
	class Wiloke_Import extends WP_Importer {
		...
		function process_menu_item( $item ) {
			...
			if ( ! $menu_slug ) {
				_e( 'Menu item skipped due to missing menu slug', 'wiloke' );
				echo '<br />';
				return;
			}
			...
			if ( ! $menu_id ) {
				printf( __( 'Menu item skipped due to invalid menu slug: %s', 'wiloke' ), esc_html( $menu_slug ) );
				echo '<br />';
				return;
			} else {
			...
			foreach ( $item['postmeta'] as $meta ){
				$arr = array( $meta['key'] => $meta['value']);
				if ( $meta['key'] === 'wiloke-nav-item-data' ){
					$aMegaMenuSettings = maybe_unserialize($meta['value']);
				}
				extract($arr);
			}
			...
			if ( isset($aMegaMenuSettings) ){
				update_post_meta($id, 'wiloke-nav-item-data', $aMegaMenuSettings);
			}
