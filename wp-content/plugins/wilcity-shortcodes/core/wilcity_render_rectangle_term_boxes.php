<?php
function wilcity_render_rectangle_term_boxes($atts){
	...
	$wrapper_class  = implode(' ', $wrapper_class) . '  ' . $atts['extra_class'] . ' wilcity-modern-term-boxes';
	?>
	<div class="<?php echo esc_attr($wrapper_class); ?>">
		...
		?>
		<div class="row" data-col-xs-gap="<?php echo esc_attr($atts['col_gap']); ?>" data-totals="<?php echo esc_attr(count($aTerms)); ?>">
			<?php
			foreach ($aTerms as $oTerm) {
				$boxClass = 'col-sm-6 col-md-6 ' . $atts['items_per_row'];
				wilcity_render_rectangle_term_box($oTerm, $boxClass, $atts);
			} ?>
