...
class RegisterSubscriptions {
	use WilokeSubmissionConfiguration;
	public $slug = 'subscriptions';
	...
	public function __construct() {
		...
		add_action('wp_ajax_wiloke_delete_subcriptions', array($this, 'deleteSubcriptions'));
		add_action('wp_ajax_wiloke_submission_export_subcriptions', array($this, 'exportFile'));
	}

	public function deleteSubcriptions(){
	...
	public function exportFile(){
		...
		header("Content-Disposition: attachment;filename=wiloke-subscriptions-statistic-".date('Y-m-D') . '.csv');
	...
	public function register() {
		add_submenu_page($this->parentSlug, esc_html__('Subscriptions', 'wiloke'), esc_html__('Subscriptions', 'wiloke'), 'administrator', $this->slug, array($this, 'showSubscriptions'));
	}
	...
	protected function fetchItems($aQuery){
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS $tblSession.* FROM $tblSession WHERE billingType='".wilokeListingToolsRepository()->get('payment:billingTypes', true)->sub('recurring')."'";
		$concat = " AND";
		...
		$sql .= " ORDER BY $tblSession.ID DESC LIMIT ".abs($this->postPerPages)." OFFSET ".abs($offset);
			...
			<?php if ( !empty($this->aSales) ) : ?>
				<div class="ui segment">
					<form action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="POST" style="display: inline-block;">
						...
						<input type="hidden" name="action" value="wiloke_submission_export_subcriptions">
						<button class="js_export button ui basic purple"><?php esc_html_e('Export Payments', 'wiloke'); ?></button>
						<button class="js_delete_subscription button ui basic red"><?php esc_html_e('Delete Checked Items', 'wiloke'); ?></button>
					</form>
				</div>
			<?php endif; ?>
