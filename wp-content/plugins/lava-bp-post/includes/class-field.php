...
class Lava_Bp_Post_Field{
		...
		if( !empty( $this->values ) && is_array( $this->values ) ) {
			foreach( $this->values as $strValue => $strLabel ) {
				$output .= sprintf(
					...
					$strValue, $strLabel,
					selected( $default == $strValue, true, false )
				);
			}
		}
	...
	public function before() { return '<div class="form-inner">'; }

	public function after() { return '</div>'; }

	public function label() { return sprintf( '<label class="field-title">%s</label>', $this->label ); }
	...
	public static function inputField( $output=null, $fieldName=false, $obj=false ) {
		...
		switch( $strElementType ) {
			...
			case 'text' : case 'email' : case 'number' :
				$output .=  sprintf(
					'<input type="%1$s" name="%2$s" class="%3$s" value="%4$s" placeholder="%5$s">',
					$obj->element_type,
	...
	public static function textareaField( $output=null, $fieldName=false, $obj=false ) {
		...
		printf(
			'<textarea name="%1$s" class="%2$s" rows="8" placeholder="%3$s" style="resize:none;">%4$s</textarea>',
			$obj->getFieldName( $fieldName ),
			$obj->getClasses(),
			$obj->placeholder,
			$obj->getValue()
	...
	public static function dropdownField( $output=null, $fieldName=false, $obj=false ) {
		return sprintf(
			'<select name="%1$s" class="%2$s">%3$s</select>',
			$obj->getFieldName( $fieldName ),
			$obj->getClasses(),
	...
	public static function mediaLibraryField( $output=null, $fieldName=false, $obj=false ) {
		if( ! is_user_logged_in() ) {
			$output = sprintf( '<div class="lava-upload-wrap" data-field="%1$s" data-multiple="false" data-value=\'%2$s\'><div class="upload-item-group"></div></div>',
				$obj->getFieldName( $fieldName ),
				json_encode( $obj->value )
			);
		}else{
			$output = sprintf(
				'<div class="lava-listing-wp-media" data-field="%4$s" data-multiple="false" data-value=\'%6$s\' data-modal-title="%1$s" data-button-select="%2$s" data-button-remove="%8$s"><input type="hidden" name="%4$s" value="%6$s"><div class="upload-preview" style="background-image:url(%7$s);"></div><div class="upload-action"><button type="button" class="action-add-item button"><i class="fa fa-plus"></i>%2$s</button></div></div>',
				$obj->dialog_title,
