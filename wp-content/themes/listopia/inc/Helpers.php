...
if (! function_exists('t_mix')) {
	 ...
	 * @throws \Exception
	 */
	function t_mix($path, $manifestDirectory = '')
	{
		...
		if (! $manifest) {
			...
			$manifest = json_decode(ob_get_contents(), true);
			ob_end_clean();
			//$manifest = json_decode(file__get_contents($manifestPath), true);
		}
