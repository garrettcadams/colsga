...
<script type="text/html" id="tmpl-activity-target-item">
	...
	<# if ( data.avatar_url ) { #>
		<img src="{{data.avatar_url}}" class="avatar {{data.object_type}}-{{data.id}}-avatar photo" alt="<?php esc_attr_e( 'Avatar', 'jvbpd' ); ?>" />
	<# } #>
	...
	<# if ( data.selected ) { #>
		<button type="button" class="bp-remove-item dashicons dashicons-no" data-item_id="{{data.id}}">
			<span class="bp-screen-reader-text"><?php echo esc_html_x( 'Remove item', 'button', 'jvbpd' ); ?></span>
		</button>
	<# } #>
</script>
