...
<h2 class="screen-heading general-settings-screen">
	<?php _e( 'Email & Password', 'jvbpd' ); ?>
</h2>

<p class="info email-pwd-info">
	<?php _e( 'Update your email and or password.', 'jvbpd' ); ?>
</p>
...
<form action="<?php echo esc_url( bp_displayed_user_domain() . bp_get_settings_slug() . '/general' ); ?>" method="post" class="standard-form" id="settings-form">
	...
	<?php if ( ! is_super_admin() ) : ?>
		...
		<label for="pwd"><?php _e( 'Current Password <span>(required to update email or change current password)</span>', 'jvbpd' ); ?></label>
		<input type="password" name="pwd" id="pwd" size="16" value="" class="settings-input small" <?php bp_form_field_attributes( 'password' ); ?>/> &nbsp;<a href="<?php echo esc_url( wp_lostpassword_url() ); ?>"><?php esc_html_e( 'Lost your password?', 'jvbpd' ); ?></a>

	<?php endif; ?>

	<label for="email"><?php _e( 'Account Email', 'jvbpd' ); ?></label>
	...
	<div class="info bp-feedback">
		...
		<p class="text"><?php esc_html_e( 'Leave password fields blank for no change', 'jvbpd' ); ?></p>
	</div>

	<label for="pass1"><?php esc_html_e( 'Add Your New Password', 'jvbpd' ); ?></label>
	...
	<label for="pass2" class="repeated-pwd"><?php esc_html_e( 'Repeat Your New Password', 'jvbpd' ); ?></label>
